...
All expression in Lisps are written in forms. One of the most common forms includes an application of function. For example, the function call in common mathematics g(x) is represented in lisp as (g x). In this example, g is the name of the function and the argument(s) follow. Furthermore, x is the only argument in this example.
...
In Lisp, car and cdr are primitive operations that can be performed on lists and other data objects. The car operation extracts the first element from the list, and the cdr operation extracts the rest of the list. For example, consider a list with three elements '(d o m); the following expression involving the car and cdr operations on said list evaluate to the following: (car  '(d o m)) evaluates to d, and (cdr '(d o m)) evaluates to '(o m). If given a pair (cons D d) and operate car and cdr on the pair, however, the evaluation would return to D and d, respectively.

Pairs are useful when building a sequence in lisp. A sequence is defined as an ordered collection of data objects. Sequences are one of the fundamental building blocks to perform recursive and other useful procedures in Lisp. As shown in the figure below, the sequence d, o, m is represented as a chain of pairs.
...
(cons d (cons o (cons m nil)))

There are also procedures that can perform operations on lists. For example, the procedure *reverse* take a list as an argument and returns a list with the same elements, but in reverse order.
